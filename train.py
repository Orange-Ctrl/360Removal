...
def make_data_loader(spec, tag=''):
    ...
    loader = DataLoader(dataset, batch_size=spec['batch_size'],
        shuffle=(tag == 'train'), num_workers=8, pin_memory=True,drop_last=True)
    return loader
...
def prepare_training():
    ...
    def compute_num_params(model, text=False):
        ...
        return num
    log('numel model: #params={}'.format(compute_num_params(model, text=True)))
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch in tqdm(train_loader, leave=False, desc='train'):
        ...
        model.set_input(inp, gt_rgb, batch['mask'],cube_rgb,cube_mask,gt_norm,vps)
...
def main(config_, save_path):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        # 在调用 train 函数时传递 svs_plane_loss
        train_loss_G, train_loss_D = train(train_loader, model, optimizer,epoch)
        ...
        # if(epoch>=300):
        #     torch.save(model.encoder.state_dict(), os.path.join(save_path, f'encoder-epoch-{epoch}.pth'))
        ...
        }, os.path.join(save_path, 'model-epoch-last.pth'))
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            if val_ssim > max_val_ssim:
                ...
                torch.save(model.encoder.state_dict(), os.path.join(save_path, 'encoder-epoch-best-ssim.pth'))
...
if __name__ == '__main__':
    ...
    save_path = os.path.join('/data/chenni/objremove_train/test—param/', save_name)
