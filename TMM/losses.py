...
class SVS(nn.Module):
    def __init__(self, device, setting):
        ...
        self.using_disp2seg = False
        self.saver = Saver("./framework_image/")
    
    def forward(self, rgb, left_depth_pred, up_depth, up_rgb, vps, epoch, mask=None):
        ...
        up_rgb_t, up_mask_t = L.splatting.render(rgb, left_depth_pred, \
        ...
        if self.using_normloss and self.training:  # 测refinenet时，先设置为False
            ...
            if self.using_disp2seg:
                ...
                out_planar = generate_planar_depth(rgb, aligned_norm, xyz, self.width, self.height, self.device, self.batch_size,
                                                   self.planar_thresh) 
...
class SVS_plane(nn.Module):
    ...
    def forward(self, rgb, pred_norm, gt_norm, vps, epoch, mask=None):
        active_loss = torch.tensor(0.0).to(self.device)
        gt_norm=F.interpolate(gt_norm, size=(256, 512), mode='bilinear', align_corners=False)
        '''Mahattan Align'''
        if self.using_normloss and self.training:
            # Compute Manhattan alignment and any required masks or thresholds
            mmap, mmap_mask, _ = compute_mmap(self.batch_size, pred_norm, vps, self.height, self.width, epoch,
                                          self.d2n_nei)  
            aligned_norm = align_smooth_norm(self.batch_size, mmap, vps, self.height, self.width)
            ...
            loss_norm_reg = 0.0
            cos = torch.nn.CosineSimilarity(dim=1, eps=1e-6)
            norm_loss_score = cos(gt_norm, aligned_norm)
            

            normloss_mask = mmap_mask  # Applying the Manhattan map mask to the norm loss
        ...
        return active_loss
       # # def forward(self,left_rgb,left_depth_pred, up_depth, up_rgb,vps,pred_norm,gt_norm, epoch, mask=None):
    # def forward(self,rgb,pred_norm,gt_norm,vps, epoch, mask=None):
