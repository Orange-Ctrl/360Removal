...
from .layers import ConvBlock, upsample, Cube2Equirec, Concat, BiProj, CEELayer, NormalHead
...
class UniFuse(nn.Module):
    ...
    def forward(self, input_equi_image, input_cube_image= torch.rand(1, 3, 128, 768).cuda()):
        #equi_image:[4, 3, 256, 512]
        #input_cube_image:[1, 3, 128, 768]
        # euqi image encoding
        ...
        equi_normal = self.equi_dec_convs["normal_head"](equi_x,1)
        equi_normal = torch.nn.functional.normalize(equi_normal, p=2, dim=1)  # Normalize the output to unit vectors
        outputs["pred_normal"] = equi_normal
