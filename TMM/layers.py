...
def depth2norm(cam_points, height, width, nei=3):  # cam_points [b,c,h*w]
    ...
    normal_vectorl2 = torch.norm(normal_vector, p=2, dim = 1) #求L2范数
    ...
    normal = - F.normalize(normal_map, dim=1, p=2)  #再一次L2标准化
    return normal
...
def compute_mmap(batch_size, norm, vps, H, W, epoch, nei):
    ...
    mmap = index.reshape(batch_size, 1, H, W)  # reshape之后每个值保留的都是0~5个方向   索引图
    ...
    mmap_mask[:, :, -8:, :] = 0
    mmap_mask[:, :, :, :8] = 0
    mmap_mask[:, :, :, -8:] = 0  # vps边缘掩膜  没计算normal的
    '''
    ...
    '''
    score = 1.633 * epoch + 700
    mmap_mask[1000*score_map < score] = 0  # vps边缘掩膜 分数小于阈值的(score)
...
def align_smooth_norm(batch_size, mmap, vps, H, W):
    ...
    mmap_label = mmap_label.repeat(3,1,1).permute(1,0,2)    #b*3*HW
    vps = vps.permute(0, 2, 1)  #Bx3x6
    smooth_norm = torch.gather(vps, 2, mmap_label)  # 根据index找到相应的align normal  HW中的值存着索引，vps的6个位置
