...
class GAN(nn.Module):
    def __init__(self, encoder_spec=None):
        ...
        # self.compute_loss = SVS_plane(self.device)
        self.compute_svs_loss = SVS_plane(
            device=self.device, 
            setting={
                'batch_size': 4,
                'd2n_nei': 5,
        ...
        self.lambda_fm = 10
        self.lambda_wavelet = 1
        self.lambda_svs = 1.0  # SVS_plane 损失的权重

        self.multi_res_training = encoder_spec['multi_res_training']
        self.optimizer_G = torch.optim.Adam(self.encoder.parameters(), lr=1e-4)
        self.optimizer_D = torch.optim.Adam(self.discriminator.parameters(), lr=5e-5)

    def set_input(self, inp, gt, input_mask, cube_rgb, cube_mask, normal, vps):
        self.input = inp.to(self.device)
        self.gt = gt.to(self.device)
        self.input_mask = input_mask.to(self.device)
        self.cube_rgb=cube_rgb.to(self.device)
        self.cube_mask=cube_mask.to(self.device)
        self.normal=normal.to(self.device)
        self.vps=vps.to(self.device)
        ...
        if self.multi_res_training:
            ...
            #self.normal=F.interpolate(self.normal, size=(size, size), mode='nearest')
            self.vps=vps

    def forward(self):
        self.pred = self.encoder([self.input,self.gt,self.input_mask,self.cube_rgb,self.cube_mask,self.normal,self.vps])
    ...
    def backward_G(self):
        ...
        try:
            # 获取预测的法线图
            pred_norm = self.encoder.get_pred_norm()
            current_epoch = getattr(self, 'current_epoch', 0)
            self.loss_svs = self.compute_svs_loss(
                rgb=self.input,
                pred_norm=pred_norm,
                gt_norm=self.normal,
                vps=self.vps,
            ...
            print(f"Warning: SVS_plane loss computation failed: {e}")
            self.loss_svs = torch.tensor(0.0).to(self.device)

        wavelet_loss = self.encoder.get_wavelet_loss()
        self.loss_G = (self.loss_perceptual + self.loss_GAN + self.feat_match_loss + 
                      self.lambda_wavelet * wavelet_loss + self.lambda_svs * self.loss_svs)
        ...
        # print(f"Generator total gradient: {total_grad:.4f}")
    ...
    def get_pred_norm(self):
        # 确保pred_norm已经被计算和保存
        if hasattr(self.encoder, 'pred_norm'):
            return self.encoder.pred_norm
        else:
            raise ValueError("pred_norm has not been computed yet.")

    def set_epoch(self, epoch):
        """设置当前训练的epoch，用于SVS_plane损失计算"""
