...
class FFCResnetBlock(nn.Module):
    ...
    def forward(self, x):
        if self.inline:
            x_l, x_g = x[:, :-self.conv1.wavlet.global_in_num], x[:, -self.conv1.wavelet.global_in_num:]
        else:
            x_l, x_g = x if type(x) is tuple else (x, 0)
...
class FFCResNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=0, n_blocks=9, res_dilation=2, norm_layer=nn.BatchNorm2d,
        ...
        assert (n_blocks >= 0)
        super().__init__()
        self.save_dir="/home/chenni/objRemove/CCQ/learnffc"
    ...
    def forward(self, input):
        ...
        save_path = os.path.join(self.save_dir, f"ffc_block_{id(self)}.pt")
