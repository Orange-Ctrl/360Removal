...
def plot_wavelet_components(ya, yh, yv, yd, save_path="wavelet_components.png"):
    """可视化小波分量（低频+水平/垂直/对角线高频）"""
    # 确保张量脱离计算图并转到CPU
    ya = ya.cpu().detach()
    yh = yh.cpu().detach()
    yv = yv.cpu().detach()
    ...
    # 显示低频分量 (Approximation)
    axs[0,0].imshow(ya[0,0].numpy(), cmap='gray')
    axs[0,0].set_title("Low-Freq (YA)", fontsize=9)
    axs[0,0].axis('off')

    # 显示水平高频分量 (Horizontal)
    axs[0,1].imshow(yh[0,0].numpy(), cmap='jet', vmin=-1, vmax=1)
    axs[0,1].set_title("Horiz-High (YH)", fontsize=9)
    axs[0,1].axis('off')

    # 显示垂直高频分量 (Vertical)
    axs[0,2].imshow(yv[0,0].numpy(), cmap='jet', vmin=-1, vmax=1)
    axs[0,2].set_title("Vert-High (YV)", fontsize=9)
    axs[0,2].axis('off')

    # 显示对角线高频分量 (Diagonal)
    axs[1,0].imshow(yd[0,0].numpy(), cmap='jet', vmin=-1, vmax=1)
    axs[1,0].set_title("Diag-High (YD)", fontsize=9)
    axs[1,0].axis('off')
...
class LWN(nn.Module):
    def __init__(self, dim, wavelet='haar', initialize=True, head=4, drop_rate=0., use_ca=False, use_sa=False):
        ...
        # 注释
        self.post_wavelet_bn = nn.BatchNorm2d(dim, affine=True)
        ...
        # conv 输入通道改为 dim * 5（4个分量 + 融合后增强分量）
        self.conv1 = nn.Conv2d(dim * 5, dim * 6, kernel_size=1)
        self.conv2 = nn.Conv2d(dim * 6, dim * 6, kernel_size=7, padding=3, groups=dim * 6)  # dw
        self.conv_mid = nn.Conv2d(dim * 6, dim * 6, kernel_size=3, padding=1, groups=dim * 6)  # 新增中间卷积
        self.conv3 = nn.Conv2d(dim * 6, dim * 4, kernel_size=1)
        # self.res_bn = nn.BatchNorm2d(dim)
        self.res_mapping = nn.Sequential(
            nn.Conv2d(512, 2048, kernel_size=3, stride=2, padding=1),  # 下采样+升维合一
            nn.BatchNorm2d(2048)
        )
        
        self.act = nn.GELU()
        self.reproj = nn.Conv2d(dim, dim, kernel_size=1)
    ...
    def forward(self, x):
        _, _, H, W = x.shape
        # 修改：加入残差连接
        # 修改：加入 3x3 深度卷积增强表达能力
        # 修改：在 wavelet 解构后对分支加通道映射再融合，形成级联增强
        ...
        yv_r = self.reproj(yv)
        yd_r = self.reproj(yd)
        dec_x = torch.cat([ya, yh, yv, yd, ya_r + yh_r + yv_r + yd_r], dim=1)  # dim*5
...
class SimpleFFT(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.conv_in = nn.Conv2d(dim, dim*2, kernel_size=1)  # 减少通道扩展
        self.conv_out = nn.Conv2d(dim*2, dim, kernel_size=1)
        self.fft_scale = nn.Parameter(torch.ones(8, 8//2 + 1))
        self.act = nn.GELU()

    def forward(self, x):
        B, C, H, W = x.shape
        x = self.conv_in(x)
        x = rearrange(x, 'b c (h p1) (w p2) -> b c h w p1 p2', p1=8, p2=8)
        x_fft = torch.fft.rfft2(x.float())
        x_fft = x_fft * self.fft_scale[None, None, None, None, :, :]
        x = torch.fft.irfft2(x_fft, s=(8, 8))
